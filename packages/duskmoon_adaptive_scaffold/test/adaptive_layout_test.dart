...
import 'package:duskmoon_adaptive_scaffold/src/adaptive_layout.dart';
import 'package:duskmoon_adaptive_scaffold/src/breakpoints.dart';
import 'package:duskmoon_adaptive_scaffold/src/slot_layout.dart';
...
void main() {
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      await tester.pumpWidget(
        slot(300, const Duration(milliseconds: 1000), tester),
      );
      expect(begin, findsOneWidget);
      expect(end, findsNothing);

      await tester.pumpWidget(
        slot(500, const Duration(milliseconds: 1000), tester),
      );
      await tester.pump();
      ...
      await tester.pumpWidget(
        slot(1300, const Duration(milliseconds: 1000), tester),
      );
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 500));
      expect(
        tester.widget<SlideTransition>(slideOut('400')).position.value,
        const Offset(-0.5, 0),
      );
      expect(
        tester.widget<SlideTransition>(slideIn('1200')).position.value,
        const Offset(-0.5, 0),
      );
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 500));
      ...
      await tester.pumpWidget(
        slot(1700, const Duration(milliseconds: 1000), tester),
      );
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 500));
      expect(
        tester.widget<SlideTransition>(slideOut('1200')).position.value,
        const Offset(-0.5, 0),
      );
      expect(
        tester.widget<SlideTransition>(slideIn('1600')).position.value,
        const Offset(-0.5, 0),
      );
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 500));
  ...
  testWidgets('adaptive layout handles internal animations correctly', (
    ...
    final Finder secondaryTestBreakpoint = find.byKey(
      const Key('Secondary Test Breakpoint'),
    );
  ...
  testWidgets('adaptive layout can adjust animation duration', (
    ...
    // Populate the smaller slot layout and let the animation settle.
    await tester.pumpWidget(
      slot(300, const Duration(milliseconds: 100), tester),
    );
    await tester.pumpAndSettle();
    expect(begin, findsOneWidget);
    expect(end, findsNothing);
    ...
    await tester.pumpWidget(
      slot(500, const Duration(milliseconds: 200), tester),
    );
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(
        tester.getBottomRight(testBreakpoint400),
        offsetMoreOrLessEquals(const Offset(792.6, 1999), epsilon: 1.0),
      );
      expect(
        tester.getTopLeft(secondaryTestBreakpoint400),
        offsetMoreOrLessEquals(const Offset(792.6, 1.0), epsilon: 1.0),
      );
      expect(
        tester.getBottomRight(secondaryTestBreakpoint400),
      ...
      expect(
        tester.getBottomRight(secondaryTestBreakpoint800),
      ...
      expect(tester.getTopLeft(testBreakpoint1200), const Offset(10, 10));
      expect(
        tester.getBottomRight(testBreakpoint1200),
        offsetMoreOrLessEquals(const Offset(1114.0, 1990.0), epsilon: 1.0),
      );
      expect(
        tester.getTopLeft(secondaryTestBreakpoint1200),
        offsetMoreOrLessEquals(const Offset(1114.0, 10.0), epsilon: 1.0),
      );
      expect(
        tester.getBottomRight(secondaryTestBreakpoint1200),
      ...
      expect(tester.getTopLeft(testBreakpoint1600), const Offset(10, 10));
      expect(
        tester.getBottomRight(testBreakpoint1600),
        offsetMoreOrLessEquals(const Offset(1459.1, 1990.0), epsilon: 1.0),
      );
      expect(
        tester.getTopLeft(secondaryTestBreakpoint1600),
...
final Finder secondaryNavigation = find.byKey(
  const Key('Secondary Navigation Small'),
);
final Finder primaryNavigation = find.byKey(
  const Key('Primary Navigation Small'),
);
...
final Finder secondaryTestBreakpoint400 = find.byKey(
  const Key('Secondary Test Breakpoint 400'),
);
final Finder secondaryTestBreakpoint800 = find.byKey(
  const Key('Secondary Test Breakpoint 800'),
);
final Finder secondaryTestBreakpoint1200 = find.byKey(
  const Key('Secondary Test Breakpoint 1200'),
);
final Finder secondaryTestBreakpoint1600 = find.byKey(
  const Key('Secondary Test Breakpoint 1600'),
);
      ...
      child: AdaptiveLayout(
        ...
        secondaryNavigation: SlotLayout(
          config: <Breakpoint, SlotLayoutConfig>{
            ...
            TestBreakpoint400(): SlotLayout.from(
              key: const Key('Secondary Navigation Medium'),
              builder: on,
            ),
            TestBreakpoint800(): SlotLayout.from(
            ...
            TestBreakpoint1200(): SlotLayout.from(
              key: const Key('Secondary Navigation Large'),
              builder: on,
            ),
            TestBreakpoint1600(): SlotLayout.from(
              key: const Key('Secondary Navigation ExtraLarge'),
              builder: on,
            ),
          },
        ),
        topNavigation: SlotLayout(
          config: <Breakpoint, SlotLayoutConfig>{
            ...
            TestBreakpoint400(): SlotLayout.from(
              key: const Key('Top Navigation1'),
              builder: on,
            ),
            TestBreakpoint800(): SlotLayout.from(
              key: const Key('Top Navigation2'),
              builder: on,
            ),
            TestBreakpoint1200(): SlotLayout.from(
              key: const Key('Top Navigation3'),
              builder: on,
            ),
            TestBreakpoint1600(): SlotLayout.from(
              key: const Key('Top Navigation4'),
              builder: on,
            ),
          },
        ),

...
class DmVizScatterChart extends StatelessWidget {
  final List<DmVizPoint> data;
  final String? xAxisLabel;
  final String? yAxisLabel;
  final DmVizMarkerShape shape;
  final double Function(DmVizPoint point)? radiusAccessor;
  final DmChartPalette? palette;

  const DmVizScatterChart({
    super.key,
    required this.data,
    this.xAxisLabel,
    this.yAxisLabel,
    this.shape = DmVizMarkerShape.circle,
    this.radiusAccessor,
    this.palette,
  });
  ...
  Widget build(BuildContext context) {
    ...
    final xDomain = _numericDomain(data.map((point) => point.x));
    final yDomain =
        _numericDomain(data.map((point) => point.y), includeZero: true);
    ...
    return XYChart(
      ...
      children: [
        ScatterSeries(
          ...
          shape: shape.toRaw(),
          color: resolvedPalette.warning,
          borderColor: resolvedPalette.warningOnColor,
          radiusAccessor: radiusAccessor == null
              ? null
              : (rawPoint) {
                  final point = DmVizPoint(
                    x: rawPoint.x,
                    y: rawPoint.y,
                    metadata: rawPoint.metadata,
                  );
                  return radiusAccessor!(point);
                },
          opacity: 0.85,
        ),
      ],
